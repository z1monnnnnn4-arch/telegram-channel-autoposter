# Telegram channel autoposter (Bot API) · v2.0
...
- Подтверждения **Да / Нет** на опасные действия
- Умное **обновление** с GitHub (проверка → установить → перезапуск)
- Глобальная **обработка ошибок** в меню (не падает молча)
- Защита от битых callback, фото, сети, lock-файла
- Частичный пост (фото без текста) + бэкапы + systemd
...
Или кнопка **СИСТЕМА → ⬆️ Обновление**:
- «У вас последняя версия» — ничего делать не нужно;
- «Доступна новая версия» → **📥 Установить** — скачает с GitHub, обновит зависимости и перезапустит бота.
